...
func trsxToLedger<T>(_ txs: [([T], [T])], _ ledger: Ledger<T>) -> [Ledger<T>] {
    ...
    for tx in txs {
        ...
        ledgers.append(Ledger<T>(positives: pos, negatives: neg, trx: nil))
    }
    return ledgers
}
    ...
    if let (f, a) = maxValue(pos) {
        if let (t, b) = maxValue(neg) {
            if a == b {
                ...
                return (Payment(from: t, to: f, payment: Double(a)/100), Ledger<Person>(positives: pos, negatives: neg, trx: ledger.trx))
            }
            if a > b {
                pos.updateValue(pos[f]! - b, forKey: f)
                neg.removeValue(forKey: t)
                return (Payment(from: t, to: f, payment: Double(b)/100), Ledger<Person>(positives: pos, negatives: neg, trx: ledger.trx))
            } else {
                pos.removeValue(forKey: f)
